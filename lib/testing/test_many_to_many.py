...
def test_contract_contracts_by_date():
    ...
    Contract.all_contracts = []  # Fix typo in the test
    ...
    expected_result = sorted([contract2, contract4], key=lambda x: x.date)
    actual_result = sorted(Contract.contracts_by_date('01/01/2001'), key=lambda x: x.date)
